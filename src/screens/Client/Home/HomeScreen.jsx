...
export default function HomeScreen() {
    ...
    const timetableInfo = useMemo(() => {
        if (!sortedTimetableData)
            return { current: null, next: null, isHoliday: false };
        const todayStr = currentTime.format("YYYY-MM-DD");
        const todaySchedule = sortedTimetableData.find(
            (day) => day.date === todayStr
        );

        if (!todaySchedule) return { current: null, next: null, isHoliday: true };

        const holidayCourse = todaySchedule.courses.find(
            (c) => c.libelle.includes("CONGÉS") || c.libelle.includes("VACANCES")
        );
        if (holidayCourse) return { current: null, next: null, isHoliday: true };

        const current = todaySchedule.courses.find((course) => {
            const start = moment(
                `${course.startCourse.date} ${course.startCourse.time}`,
                "YYYY-MM-DD HH:mm"
            );
            const end = moment(
                `${course.endCourse.date} ${course.endCourse.time}`,
                "YYYY-MM-DD HH:mm"
            );
    ...
    const progress = useMemo(() => {
        ...
        const start = moment(
            `${timetableInfo.current.startCourse.date} ${timetableInfo.current.startCourse.time}`,
            "YYYY-MM-DD HH:mm"
        );
        const end = moment(
            `${timetableInfo.current.endCourse.date} ${timetableInfo.current.endCourse.time}`,
            "YYYY-MM-DD HH:mm"
        );
    ...
    const timeToNext = useMemo(() => {
        ...
        const start = moment(
            `${timetableInfo.next.startCourse.date} ${timetableInfo.next.startCourse.time}`,
            "YYYY-MM-DD HH:mm"
        );
    ...
    const averages = useMemo(() => {
        ...
        const subjectAverages = subjects
            .map((s) => {
                ...
                const validGrades = gradesList.filter(
                    (g) => !g.notSignificant && g.data.grade !== null
                );
                if (validGrades.length === 0)
                    return { name: s.libelle, average: null, count: 0 };
                let totalPoints = 0,
                    totalCoef = 0;
                ...
                return {
                    name: s.libelle,
                    average: (totalPoints / totalCoef).toFixed(2),
            ...
            subjectAverages.length > 0
                ? (
                      subjectAverages.reduce(
                          (acc, s) => acc + parseFloat(s.average),
                          0
        ...
        return {
            ...
            period: latestPeriodCode.replace(/\D/g, "") || periodCodes.length,
        };
    }, [sortedGradesData]);
    ...
    const homeworkInfo = useMemo(() => {
        ...
        const futureDates = dates.filter((date) =>
            moment(date).isSameOrAfter(currentTime, "day")
        );
        return futureDates.map((date) => ({
            ...
            tasks:
                sortedHomeworksData[date]?.map((t) => {
                    let taskContent = "Pas de description";
                    const subjectName = t.discipline?.name || t.matiere || "Inconnu";
                    try {
                        // Support both old (aFaire/devoir) and new (homeworksContent) structures
                        const rawContent =
                        ...
                        taskContent = "Description indisponible";
                    }
                    return {
                        subject: subjectName,
                        task: taskContent,
    ...
    const examInfo = useMemo(() => {
        ...
        return Object.values(sortedGradesData)
            .flatMap((p) =>
                p.groups.flatMap((g) =>
                    (g.isDisciplineGroup ? g.disciplines : [g]).flatMap(
                        (d) => d.grades || []
                    )
                )
            )
            .filter(
                (g) => g.isExam || g.homeworkType?.toLowerCase().includes("contrôle")
            )
                ...
                >
                    ...
                    {timetableInfo.isHoliday ? (
                        <View style={[styles.card, styles.holidayCard]}>
                            ...
                            >
                                ...
                                <Text style={styles.holidayText}>
                                    CanardMan en profite pour faire un petit somme...
                                    💤💤💤
                                </Text>
                        ...
                        <View style={[styles.card, styles.courseCard]}>
                            <View style={styles.cardHeader}>
                                <Text
                                    style={[styles.cardTitle, { color: "#5C71FA" }]}
                                >
                                    ● EN COURS
                                </Text>
                            </View>
                            {timetableInfo.current ? (
                                <>
                                    <View style={styles.courseRow}>
                                        ...
                                        >
                                            → {timetableInfo.current.endCourse.time}
                                        </Text>
                                    </View>
                                    <View style={styles.progressBarBg}>
                                        <LinearGradient
                                            colors={["#5C71FA", "#3F51B5"]}
                                            start={{ x: 0, y: 0 }}
                                            end={{ x: 1, y: 0 }}
                                            style={[
                                                styles.progressBar,
                            ...
                            {timetableInfo.next && (
                                <>
                                    ...
                                    <View style={styles.courseRow}>
                                        <Text
                                            style={[
                                                styles.nextSubjectName,
                                                { color: "white" },
                                            ]}
                                            numberOfLines={1}
                                        >
                                            {timetableInfo.next.libelle}
                                        </Text>
                                        <View style={styles.roomBadge}>
                                            <Text style={styles.roomText}>
                                                {timetableInfo.next.room || "???"}
                                            </Text>
                                        </View>
                                    </View>
                                    <View style={styles.courseFooter}>
                                        ...
                                        >
                                            {timetableInfo.next.startCourse.time} /{" "}
                                            {timetableInfo.next.endCourse.time}
                                        </Text>
                                    </View>
                                </>
                            )}
                    ...
                    <View style={[styles.card, styles.averageCard]}>
                        <Text
                            style={[
                                styles.cardTitle,
                                { color: "#5C71FA", marginBottom: 5 },
                            ]}
                        >
                            MOYENNE GÉNÉRALE
                        </Text>
                        <View style={styles.averageRow}>
                            <Text style={[styles.averageValue, { color: "white" }]}>
                                {averages.general}
                            </Text>
                        ...
                        >
                            Période {averages.period} • {moment().year()}
                        </Text>
                    </View>
                            ...
                            {averages.subjects.slice(0, 3).map((subject, index) => (
                                ...
                                >
                                    ...
                                    <View style={styles.smallCardValueRow}>
                                        ...
                                        <Text style={styles.smallCardTotal}>
                                            /20
                                        </Text>
                    ...
                    <View style={styles.toggleContainer}>
                        <TouchableOpacity
                            onPress={() => setActiveTab("homeworks")}
                            style={
                                activeTab === "homeworks"
                                    ? styles.toggleActive
                    ...
                    {activeTab === "homeworks" ? (
                        homeworkInfo.slice(0, 2).map((day, idx) => (
                            <View
                                key={idx}
                            ...
                            >
                                <View style={styles.homeworkHeader}>
                                    ...
                                    <View style={styles.restantsBadge}>
                                        <Text style={styles.restantsText}>
                                            {day.tasks.length} restants
                                        </Text>
                                    </View>
                                </View>
                                <View style={styles.nextCourseSeparator}>
                                    ...
                                    <View style={styles.timeBadge}>
                                        <Text style={styles.timeBadgeText}>
                                            {day.daysAway === 0
                                                ? "Aujourd'hui"
                                                : `Dans ${day.daysAway} jour${day.daysAway > 1 ? "s" : ""}`}
                                        </Text>
                                    </View>
                                    <View style={styles.separatorLine} />
                                </View>
                                {day.tasks.map((task, tIdx) => (
                                    <View key={tIdx} style={styles.homeworkItem}>
                                        <Text
                                            style={[
                                                styles.homeworkSubject,
                                                { color: task.color || "#9C27B0" },
                                            ]}
                                        >
                        ...
                        <View style={[styles.card, styles.homeworkCard]}>
                            ...
                            {examInfo.length > 0 ? (
                                examInfo.map((exam, idx) => (
                                    <View key={idx} style={styles.homeworkItem}>
                                        <View
                                            style={{
                                                flexDirection: "row",
                                        ...
                                        >
                                            ...
                                            >
                                                {exam.disciplineName.toUpperCase()}
                                            </Text>
                                            <Text
                                                style={{
                                                    color: "white",
                                            ...
                                            >
                                                {exam.data.grade}/{exam.data.outOf}
                                            </Text>
                                        </View>
                                        <Text
                                            style={[
